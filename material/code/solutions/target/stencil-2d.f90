program stencil_2d
  ...
  !$omp target enter data map(to: u(1:nx, 1:ny), u_new(1:nx, 1:ny))
  ...
  do it = 1, nit
    ...
    ! update all inner points and accumulate the residual in the same loop
    !$omp target teams distribute parallel do collapse(2) &
    !$omp&  reduction(+ : res) private(local_res)
    do j = 2, ny - 1
      do i = 2, nx - 1
    ...
    ! swap source and destination pointers on the host
    tmp   => u
    u     => u_new
    u_new => tmp
  end do
  ...
  !$omp target exit data map(from: u(1:nx, 1:ny), u_new(1:nx, 1:ny))
  ...
  ! compute the final residual on the host
  res = 0.0_real64
  do j = 2, ny - 1
    do i = 2, nx - 1
      local_res = 4 * u(i, j) - (u(i-1, j) + u(i+1, j) + u(i, j-1) + u(i, j+1))
